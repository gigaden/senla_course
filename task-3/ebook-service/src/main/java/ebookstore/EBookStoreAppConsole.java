...
public class EBookStoreAppConsole {
    ...
    public static void main(String[] args) {
        try {
            ...
            container.registerBean(
                    ...
                    DataSerializer.<BookRepository>deserializeData(BOOKS_FILE)
                            .orElse(new InMemoryBookRepository())
            );

            container.registerBean(
                    ClientRepository.class,
                    DataSerializer.<ClientRepository>deserializeData(CLIENTS_FILE)
                            .orElse(new InMemoryClientRepository())
            );

            container.registerBean(
                    OrderRepository.class,
                    DataSerializer.<OrderRepository>deserializeData(ORDERS_FILE)
                            .orElse(new InMemoryOrderRepository())
            );

            container.registerBean(
                    BookRequestRepository.class,
                    DataSerializer.<BookRequestRepository>deserializeData(REQUESTS_FILE)
                            .orElse(new InMemoryBookRequestRepository())
            );
            ...
            Runtime.getRuntime().addShutdownHook(new Thread(() -> {
                ...
                ebookStoreApp.saveData(container);
            }));
    ...
    private void saveData(DIContainer container) {
        BookRepository bookRepository = container.getBean(BookRepository.class);
        ClientRepository clientRepository = container.getBean(ClientRepository.class);
        OrderRepository orderRepository = container.getBean(OrderRepository.class);
        BookRequestRepository requestRepository = container.getBean(BookRequestRepository.class);
